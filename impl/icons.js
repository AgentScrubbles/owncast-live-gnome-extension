/**
 * Icon caching module for Owncast instances
 ...
 */
function getCacheDir(GLib) {
    return GLib.build_filenamev([GLib.get_user_cache_dir(), 'owncastlive-extension']);
}
 ...
 */
function ensureCacheDir(GLib, Gio) {
    const cacheDir = getCacheDir(GLib);
 ...
 */
function getCachedIconPath(GLib, Gio, instanceUrl) {
    ensureCacheDir(GLib, Gio);
    const cacheDir = getCacheDir(GLib);
 ...
 */
async function downloadAndCacheIcon(Soup, GLib, Gio, instanceUrl, iconUrl) {
    ensureCacheDir(GLib, Gio);
    const cacheDir = getCacheDir(GLib);
 ...
 */
async function getIcon(Soup, GLib, Gio, instanceUrl, iconUrl) {
    // Check cache first
    const cached = getCachedIconPath(GLib, Gio, instanceUrl);
    if (cached) {
        return cached;
    }
    ...
    if (iconUrl) {
        return await downloadAndCacheIcon(Soup, GLib, Gio, instanceUrl, iconUrl);
    }
 ...
 * @param {Object} Gio - The Gio module
 */
function clearIconCache(GLib, Gio) {
    try {
        const cacheDir = getCacheDir(GLib);
        const dir = Gio.File.new_for_path(cacheDir);
        if (dir.query_exists(null)) {
...
export { getCachedIconPath, downloadAndCacheIcon, getIcon, clearIconCache };
